...
const ParametersDetails = ({
  ...
  return (
    <div className="w-full -mt-28 pr-8 pl-4" style={ParametersContainerStyle(width)}>
      <div
        className={` p-8 ${errorResponse.error_code === 404 ? "w-[35%]" : ""}`}
      >
        {errorResponse.error_code !== 404 && eventWithNewNames.length > 0 ? (
          <div className={`card ${isInDarkMode ? "bg-gray-900"  : "bg-slate-100"} p-2 rounded-lg w-[100%] flex flex-row flex-wrap justify-between`}>
